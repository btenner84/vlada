...
async function testOpenAIAPI() {
  ...
  try {
    ...
    console.log('Initialized OpenAI client, making test request...');
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        ...
        { role: "user", content: "Hello, this is a test message. Reply with 'OpenAI API is working correctly!'" }
      ],
      max_tokens: 50,
    });
...
// Function to test service categorization
async function testServiceCategorization() {
  console.log('\nTesting service categorization...');
  try {
    // Import the documentProcessing module directly
    const documentProcessing = require('../utils/documentProcessing');
    ...
    // Test the categorization directly with OpenAI
    const openai = new OpenAI({
      apiKey: process.env.OPENAI_API_KEY,
    });
...
Please categorize this service into one of these six categories. Respond in JSON format with the following structure:
{
  ...
  "reasoning": "Brief explanation of why this category is appropriate"
    ...
    console.log('\nService categorization test completed!');
    return true;
  } catch (error) {
...
async function runTests() {
  ...
  if (apiTest) {
    ...
    await testServiceCategorization();
  }
  
  console.log('\nTests completed!');
  process.exit(apiTest ? 0 : 1);
}
